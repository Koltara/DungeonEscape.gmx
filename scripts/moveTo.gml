...
if xVelFrac!=0
    if round(1/xVelFrac)!=0
         xVelInteger=(oGameController.time mod round(1/xVelFrac)=0)
if yVelFrac!=0
    if round(1/yVelFrac)!=0
         yVelInteger=(oGameController.time mod round(1/yVelFrac)=0)
...
if xVelInteger>0
  ...
  {
    solidId=getIdCollisionRight(1, oSolid)
...
if xVelInteger<0
  for(x=x;x>mtXPrev+xVelInteger;x-=1)
  {
    solidId=getIdCollisionLeft(1, oSolid)
    //if there is a collision with a solid
    if solidId>0
    {
        break;
